...
public class HibernateTest {
	...
	Criteria criteria=session.createCriteria(UserDetails.class);//Tüm kayýtlarý getirir criteria nesnesinde tutar.kriter eklenmemiþ hali
	
	List<UserDetails> users1=(List<UserDetails>) criteria.list();
	
	for(UserDetails i: users1)
	{
		System.out.println(i.getUserId()+".id "+" "+i.getUserName());
	}
